...
def get_file():
    ...
    while True:
        if file_dir == '':
            files, filetype = QFileDialog.getOpenFileNames(filter="CSV File (*.CSV);;All Files (*)")
        else:
            files, filetype = QFileDialog.getOpenFileNames(directory=file_dir,filter="CSV File (*.CSV);;All Files (*)")
        for i in files:
    ...
    def __init__(self):      
        ...
        for key, value in global_list.items():
            ...
            for i in value:
                ...
                global_data_list.append(  ( '{}_Ex{}'.format(key_name,ex_nm)   ,df) )
