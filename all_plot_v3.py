    ...
    def __init__(self):      
        ...
        while True:
            if file_dir == '':
                ...
                files, filetype = QFileDialog.getOpenFileNames(directory=file_dir)
            for i in files:
        ...
        for key, value in global_list.items():
            ...
            for i in value:
                ...
                data_list.append(   ( '{}_Ex{}'.format(key_name,ex_nm)   ,df)  )
            data_top_list=[]
            ...
            while True:
                ...
                if LimitRange:
                    ...
                    while True:
                        smooth = input('advandce smooth  obb num  ')
                        if smooth =='q':
                            break
                        tm=int(smooth)
                        if tm%2 == 1 :
                    ...
                    myplt2=MyPltQWidget('{}_normal_{}_smooth_{}_{}'.format(key_name,normalize,smooth_m,smooth_k))
                    myplt2.set_df(data_list2)
                    myplt2.setWindowTitle(key)
                    plt_list.append(myplt2)
                    data_top_list.append(('normal_{}_smooth_{}_{}'.format(normalize,smooth_m,smooth_k),data_list2))
                    for ip in plt_list:
                        ip.show()

            
                ooo = input('is quit open origin ')
                if ooo =='q':
                    quit()
                if  ooo.isdigit():
                    if int(ooo) == 1:
